...
class ImageData:
    ...
    def delete_object(self, id: str) -> None:
        """
        Delete an object by its ID using the objects manager.
        """
    ...
    def find_images_to_process(self):
        """
        Find images that need to be processed by comparing source folder and destination file.
        """
        ...
        if os.path.exists(self.destination_file):
            try:
                with open(self.destination_file, 'r') as file:
                    ...
                    processed_images = {image['image_name']
                                        for image in existing_data}
                    all_files = [
                        f for f in all_files if f not in processed_images]
            except (json.JSONDecodeError, IOError) as e:
                print(
                    f"Error reading or parsing {self.destination_file}: {e}")
